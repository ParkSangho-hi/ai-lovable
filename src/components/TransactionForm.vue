<template>
  <div class="card card-soft p-4">
    <h3 class="h4 fw-bold mb-4">내역 추가</h3>
    <form @submit.prevent="submit" class="vstack gap-3">
      ...
      <div>
        ...
        <input v-model="form.description" class="form-control form-control-lg rounded-3" placeholder="간단한 메모" />
      </div>

      <button class="btn btn-kb btn-lg rounded-3 mt-2">추가하기</button>
    </form>
  </div>
</template>
...
const incomeCategories = ['급여', '보너스', '사업소득', '기타수입']
const expenseCategories = ['식비', '교통비', '쇼핑', '공과금', '의료비', '문화생활', '기타지출']
